...
fun CreateBlogEntryCommand.toCreateBlogEntry() =
    CreateBlogEntry(
...
fun CreateGuestBookCommand.toCreateGuestBook() =
    CreateGuestBook(
        title = requireNotNull(title) { Constants.TITLE_CANNOT_BE_NULL },
        userId = requireNotNull(userId) { "User ID cannot be null!" },
    )

fun CreateGuestBookEntryCommand.toCreateGuestBook() =
...
fun CreateUserCommand.toCreateUser() =
    CreateUser(
        addressId = addressId,
        personId = personId,
        username = username,
...
fun GuestBookEntryDto.toGuestBookEntry(parent: GuestBook?) =
    GuestBookEntry(
        persistent = persistentDto.toPersistent(),
...
fun com.github.jactor.rises.shared.api.UserType.toModel(): UserType =
    UserType.entries
        .firstOrNull { it.name == this.name } ?: error("User type ${this.name} not found!")

fun PersistentDto.toPersistent() =
    Persistent(
        id = id,
...
fun UpdateBlogTitleCommand.toUpdateBlogTitle() =
    UpdateBlogTitle(
        blogId = requireNotNull(blogId) { "Blog ID cannot be null!" },
        title = requireNotNull(title) { Constants.TITLE_CANNOT_BE_NULL },
    )
