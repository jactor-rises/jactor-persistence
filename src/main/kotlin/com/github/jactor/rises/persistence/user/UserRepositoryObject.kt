...
object UserRepositoryObject : UserRepository {
    ...
    override fun findById(id: UUID): UserDao? =
        Users
            .selectAll()
    ...
    override fun findByPersonId(id: UUID): List<UserDao> =
        Users
            .selectAll()
            .andWhere { Users.personId eq id }
            .map { it.toUserDao() }

    override fun findByUsername(username: String): UserDao? =
        Users
            .selectAll()
            .andWhere { Users.username eq username }
            .singleOrNull()
    ...
    override fun findUsernames(userType: List<UserType>): List<String> =
        run {
    ...
    override fun save(userDao: UserDao): UserDao =
        when (userDao.isPersisted) {
            true -> update(user = userDao)
            false -> insert(user = userDao)
        }

    private fun insert(user: UserDao): UserDao =
        Users
            .insertAndGetId { row ->
            ...
            .let { newId -> user.also { it.id = newId } }

    private fun update(user: UserDao): UserDao =
        Users
            .update(
