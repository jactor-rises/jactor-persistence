...
internal class GuestBookServiceTest {
    ...
    private val guestBookServiceToTest: GuestBookService =
    ...
    fun `should map guest book to a dto`() =
        runTest {
            val guestBookDao =
                initGuestBook(title = "@home").withId().toGuestBookDao().apply {
    ...
    @Test
    fun `should save GuestBookDto as GuestBookDao`() =
        runTest {
    ...
    @Test
    fun `should save GuestBookEntryDto as guestBookEntryDao`() =
        runTest {
            ...
            every { userRepositoryMockk.findById(id = guestBook.userId!!) } returns user.toUserDao()
            every { guestBookRepositoryMockk.findGuestBookById(id = guestBookEntry.guestBookId!!) } returns
                guestBook
                    .toGuestBookDao()
