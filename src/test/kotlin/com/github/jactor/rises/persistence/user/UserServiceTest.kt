...
internal class UserServiceTest {
    ...
    @Test
    fun `should map a user dao to a dto`() =
        runTest {
            val addressDto = initAddress()
            val personDto = initPerson(address = addressDto)

            every { userRepositoryMockk.findByUsername("jactor") } returns
                initUser(
                    person = personDto,
                    username = "jactor",
                ).toUserDao()
    ...
    @Test
    fun `should also map a user dao to a dto when finding by id`() =
        runTest {
            val uuid = UUID.randomUUID()
            val addressDto = initAddress()
            val personDto = initPerson(address = addressDto)

            every { userRepositoryMockk.findById(uuid) } returns
                initUser(
                    person = personDto,
                    username = "jactor",
                ).toUserDao()
